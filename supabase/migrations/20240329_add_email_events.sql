...
-- Add tracking columns to emails table if they don't exist
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'emails' AND column_name = 'opened') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'emails' AND column_name = 'clicked') THEN
        ALTER TABLE emails ADD COLUMN clicked BOOLEAN DEFAULT FALSE;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'emails' AND column_name = 'clicked_at') THEN
